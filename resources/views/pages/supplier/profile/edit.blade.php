            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="category" id="category" required
                        ...
                        @foreach($categories as $value => $label)
                            <option value="{{ $value }}" {{ old('category', $supplier->category->value) === $value ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="short_description" id="short_description" rows="3" required
                              maxlength="{{ $supplier->tier->maxDescriptionLength() }}"
                              class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('short_description', $supplier->short_description) }}</textarea>
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <textarea name="long_description" id="long_description" rows="6"
                              maxlength="{{ $supplier->tier->maxDescriptionLength() }}"
                              class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('long_description', $supplier->long_description) }}</textarea>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="contact_phone" id="contact_phone" value="{{ old('contact_phone', $supplier->contact_phone) }}"
                               class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    </div>
                </div>
